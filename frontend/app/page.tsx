    ...
    <div className="space-y-10">
      <section className="rounded-2xl border border-sky-100 bg-gradient-to-r from-white via-sky-50 to-blue-100 px-8 py-10 text-slate-900 shadow-lg">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            className="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-sky-700 shadow hover:bg-sky-50"
        ...
        {scenarioCatalog.map((section) => (
          <div key={section.discipline} className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs uppercase tracking-wide text-sky-700">新しいセッションを開始</span>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-2">
              {section.scenarios.map((scenario) => (
                <div
                  key={scenario.id}
                  className="flex min-w-[280px] flex-1 flex-col justify-between rounded-xl border border-sky-200 bg-white p-5 shadow-sm md:min-w-[320px]"
                >
                  <div className="space-y-2">
                    <p className="text-xs uppercase tracking-wide text-slate-500">{scenario.discipline}</p>
                  ...
                  <div className="mt-4 flex items-center justify-end gap-3">
                    ...
                    {savedByScenario[scenario.id] ? (
                      <Link
                        className="rounded-md border border-sky-100 px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-sky-50"
      ...
      <section className="rounded-2xl border border-sky-100 bg-white px-6 py-8 shadow-sm">
        <div className="space-y-4">
          ...
          <ol className="grid gap-4 text-base text-slate-800 md:grid-cols-3">
            ...
            <li className="rounded-xl border border-sky-100 bg-sky-50 p-5 shadow-[0_1px_2px_rgba(0,0,0,0.06)]">
              ...
              <p className="mt-2 text-lg font-semibold text-slate-900">チャットしながら整理</p>
              <p className="mt-2 text-slate-700">
            ...
            <li className="rounded-xl border border-sky-100 bg-sky-50 p-5 shadow-[0_1px_2px_rgba(0,0,0,0.06)]">
              ...
              <p className="mt-2 text-lg font-semibold text-slate-900">評価と振り返り</p>
              <p className="mt-2 text-slate-700">
